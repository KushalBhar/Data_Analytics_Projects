...
# Local files
from  data_loader import load_relevant_data, us_state_abbrev
...
def create_usa_figure(df, filename, day):
	...
	# print(f"DEBUG : {df.shape}")
	# print(f"DEBUG : {df.dtypes}")
	# print(f"DEBUG : {type(day)}")
	# print(f"DEBUG : {df.columns.tolist()}")
	

	date_index = df.columns.get_loc(day)

	df['Cases'] = df.iloc[:, date_index]
